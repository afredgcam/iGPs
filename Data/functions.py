...
def gen_iSE2_track(Tmax,d,s2,l,µ,Σ,dim=2,dt=1,first_is_last=False):
    ...
    sqrt_C = cholesky(C[1:,1:])
    x[-d:-1,:] = sqrt_C.T @ norm.rvs(size=[d-1,dim])
    
    # sample initial location
    X_star = Σ**0.5 * norm.rvs(size=[1,dim]) + µ
    x[-d:,:] += X_star
...
def gen_iSE1_track(Tmax,d,s2,l,µ,Σ,dim=2,dt=1,first_is_last=False):
...
def gen_CV_track(Tmax,Q,prior_var,dim=2,dt=1):
    
    # initialise track object
    x = np.zeros([Tmax,2*dim]) # need velocities as well
    
    # start track
    sqrt_C = cholesky(prior_var)
    x[:1,:] = norm.rvs(size=[1,2*dim]) @ sqrt_C
    
    # common quantities
    F1 = np.array([[1,dt],[0,1]])
    F = np.kron(np.eye(dim),F1)
    W1 = Q**0.5 * np.array([[dt**2/2,dt]]).T
    W = np.kron(np.eye(dim),W1)
    
    for k in range(1,Tmax):
        # sample next state
        x_new = F @ x[k-1:k,:].T + W @ norm.rvs(size=[dim,1])
        x[k,:] = x_new.flatten()
